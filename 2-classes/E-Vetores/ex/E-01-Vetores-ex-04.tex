%------------------------------------------------------------------------------%
\begin{question}
  Avalie e esboce o vetor
  \[
    v(x, y) = \vetor{2x+y-1}{1+y-x}
  \]
  nos pontos
  \(( 0,  0)\),
  \(( 1,  1)\),
  \(( 1, -1)\)
\end{question}

%------------------------------------------------------------------------------%
\begin{resolution}{Ponto \((0, 0)\)}
\begin{columns}
\column{0.5\linewidth}
  \begin{align*}
    \onslide<+->{v(0, 0)}
    \onslide<+->{& = \vetor{2x+y-1}{1+y-x} \evalat{(0, 0)}{} \\}
    \onslide<+->{& = \vetor{2\times 0+0-1}{1+0-0} \\}
    \onslide<+->{& = \vetor{-1}{1}}
  \end{align*}
\column{0.5\linewidth}
  \onslide<+->{\includegraphics{E-01-Vetores-ex-04-1}}
\end{columns}
\end{resolution}

%------------------------------------------------------------------------------%
\begin{resolution}{Ponto \((1, 1)\)}
\begin{columns}
\column{0.5\linewidth}
  \begin{align*}
    \onslide<+->{v(1, 1)}
    \onslide<+->{& = \vetor{2x+y-1}{1+y-x} \evalat{(1, 1)}{} \\}
    \onslide<+->{& = \vetor{2\times 1+1-1}{1+1-1} \\}
    \onslide<+->{& = \vetor{2}{1}}
  \end{align*}
\column{0.5\linewidth}
  \onslide<+->{\includegraphics{E-01-Vetores-ex-04-2}}
\end{columns}
\end{resolution}

%------------------------------------------------------------------------------%
\begin{resolution}{Ponto \((1, -1)\)}
\begin{columns}
\column{0.5\linewidth}
  \begin{align*}
    \onslide<+->{v(1, -1)}
    \onslide<+->{& = \vetor{2x+y-1}{1+y-x} \evalat{(1, -1)}{} \\}
    \onslide<+->{& = \vetor{2\times 1+(-1)-1}{1+(-1)-1}  \\}
    \onslide<+->{& = \vetor{0}{-1}}
  \end{align*}
\column{0.5\linewidth}
  \onslide<+->{\includegraphics{E-01-Vetores-ex-04-3}}
\end{columns}
\end{resolution}

%------------------------------------------------------------------------------%
\framedgraphic*{Campo Vetorial \(\times 0,1\)}{E-01-Vetores-ex-04-4}{}

%------------------------------------------------------------------------------%
